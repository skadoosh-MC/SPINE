...
class XPower(Power):        
    def get_growth(self):
        """
        Calculates the cosmological parameter instance required for xtilde calculation
        Retruns
        -----------
        ...
        cospar ={'omega_c': (self.omega_m - self.omega_b)*self.h**2,
        ...
        # define an instance of GrowthCalculator
        growth_val = GrowthCalculator(cospar)
        return growth_val

    def gaussian_kernel(self, tau, tau_prime, tau_s):
        """
        Computes a normalised Gaussian kernel for integration.
        Returns
        -------
        gk: float
